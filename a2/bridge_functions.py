...
def find_bridges_in_radius(bridges: list[list], lat: float, lon: float,
    ...
    for bridge in bridges:
        ...
        if distance <= radius and not bridge[COLUMN_ID] in exclusions:
...
def map_route(bridges: list[list], lat: float, lon: float,
    ...
    """
    sequence = []
    bridges_in_radius = find_bridges_in_radius(bridges, lat, lon, 
                                               radius, sequence) 
    while len(sequence) <= max_bridges and len(bridges_in_radius) > 0:
        worst_id = find_worst_bci(bridges, bridges_in_radius)
        sequence.append(worst_id)
        bridge = find_bridge_by_id(bridges, worst_id)
...
def clean_span_data(raw_spans: str) -> list[float]:
    ...
    """
    result = []
    splitted = raw_spans.split(' ')
    spans = splitted[-1].split(';')
    spans.pop()
    for span in spans:
        new = span.split('=')
        result.append(float(new[1]))
    return result
